...
export class MCPController {
  ...
  async getPlaylist(req: Request, res: Response) {
    try {
      ...
      if (!playlist.isPublic && req.isAuthenticated() && playlist.userId !== (req.user as any).id) {
        const response: MCPResponse<any> = {
          success: false,
  ...
  async createPlaylist(req: Request, res: Response) {
    try {
      ...
      const userId = (req.user as any).id;
      const { name, description, isPublic } = req.body as MCPCreatePlaylistRequest;
  ...
  async updatePlaylist(req: Request, res: Response) {
    try {
      ...
      // Extract update data
      const { name, description, isPublic } = req.body as MCPUpdatePlaylistRequest;
  ...
  async addMovieToPlaylist(req: Request, res: Response) {
    try {
      ...
      const userId = (req.user as any).id;
      const { playlistId, movieId, notes, sortOrder } = req.body as MCPAddToPlaylistRequest;
      ...
      const nextSortOrder = sortOrder !== undefined ? sortOrder : (existingItems.length + 1);
  ...
  async getSentFriendRequests(req: Request, res: Response) {
    try {
      ...
      const sentRequests = friendships.filter(f => 
        f.userId === userId && f.status === "pending"
      );
  ...
  async sendFriendRequest(req: Request, res: Response) {
    try {
      ...
      if (existingFriendship) {
        const statusMessage = existingFriendship.status === "accepted" 
  ...
  async acceptFriendRequest(req: Request, res: Response) {
    try {
      ...
      const updatedFriendship = await storage.updateFriendshipStatus(friendshipId, "accepted");
  ...
  async rejectFriendRequest(req: Request, res: Response) {
    try {
      ...
      // Update friendship status
      const updatedFriendship = await storage.updateFriendshipStatus(friendshipId, "rejected");
  ...
  async sharePlaylist(req: Request, res: Response) {
    try {
      ...
      const userId = (req.user as any).id;
      const { playlistId, sharedWithUserId, canEdit } = req.body as MCPSharePlaylistRequest;
      ...
      const existingShares = await storage.getPlaylistShares(Number(playlistId));
      const alreadyShared = existingShares.some(share => share.sharedWithUserId === Number(sharedWithUserId));
    ...
    try {
      ...
      const enhancedSharedWatches = await Promise.all(
        sharedWatches.map(async (sharedWatch) => {
          ...
          const initiatedByUser = await storage.getUser(sharedWatch.initiatedByUserId);
          const watchedWithUser = await storage.getUser(sharedWatch.watchedWithUserId);
  ...
  async addSharedWatch(req: Request, res: Response) {
    try {
      ...
      const userId = (req.user as any).id;
      const { movieId, watchedWithUserId, rating, notes } = req.body as MCPSharedWatchRequest;
  ...
  async removeSharedWatch(req: Request, res: Response) {
    try {
      ...
      if (sharedWatch.initiatedByUserId !== userId && sharedWatch.watchedWithUserId !== userId) {
        const response: MCPResponse<any> = {
          success: false,
