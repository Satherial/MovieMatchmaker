...
playlistSharesRouter.get("/", async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.status(500).json({ error: "Failed to retrieve shared playlists" });
  }
});
...
playlistSharesRouter.get("/:playlistId", async (req: Request, res: Response) => {
  if (!req.isAuthenticated()) {
    return res.status(401).json({ error: "Not authenticated" });
  }
  ...
  try {
    ...
    if (playlist.userId !== req.user!.id) {
      return res.status(403).json({ error: "You don't have permission to view this playlist's shares" });
    }
    ...
    return res.status(500).json({ error: "Failed to retrieve playlist shares" });
  }
});
...
playlistSharesRouter.post("/", async (req: Request, res: Response) => {
  ...
  try {
    const { playlistId, sharedWithUserId, canEdit } = z.object({
      playlistId: z.number(),
    ...
    // Check if user owns this playlist
    if (playlist.userId !== req.user!.id) {
      return res.status(403).json({ error: "You don't have permission to share this playlist" });
    }
...
// Remove a share
playlistSharesRouter.delete("/:playlistId/:userId", async (req: Request, res: Response) => {
  if (!req.isAuthenticated()) {
    return res.status(401).json({ error: "Not authenticated" });
  }
  ...
  try {
    ...
    // Check if user owns this playlist
    if (playlist.userId !== req.user!.id) {
      return res.status(403).json({ error: "You don't have permission to modify this playlist's shares" });
    }
