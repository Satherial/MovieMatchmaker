...
export default function PlaylistDetailPage() {
  ...
  const [selectedMovie, setSelectedMovie] = useState<(Movie & { playlistItem: PlaylistItem | null }) | null>(null);
  ...
  const removeMovieMutation = useMutation({
    mutationFn: async (movieId: number) => {
      await apiRequest("DELETE", `/api/playlists/${playlistId}/movies/${movieId}`);
    },
    onSuccess: () => {
  ...
  const updateNotesMutation = useMutation({
    mutationFn: async ({ itemId, notes }: { itemId: number; notes: string }) => {
      const res = await apiRequest("PUT", `/api/playlists/${playlistId}/movies/${itemId}`, { notes });
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [`/api/playlists/${playlistId}`] });
  ...
  const reorderPlaylistMutation = useMutation({
    mutationFn: async (itemIds: number[]) => {
      const res = await apiRequest("PUT", `/api/playlists/${playlistId}/reorder`, { itemIds });
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [`/api/playlists/${playlistId}`] });
  ...
  const handleEditNotes = (movie: Movie & { playlistItem: PlaylistItem | null }) => {
    ...
    <Container className="py-8">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="text-3xl font-bold flex items-center gap-2">
                {playlist.name}
                {playlist.isPublic ? (
                  <Badge variant="outline" className="ml-2 text-green-600 flex items-center gap-1">
                    <Globe className="h-3 w-3" />
                    Public
                  </Badge>
                ) : (
                  <Badge variant="outline" className="ml-2 text-amber-600 flex items-center gap-1">
        ...
        <div className="flex flex-col sm:flex-row text-sm text-muted-foreground gap-4">
          ...
          {playlist.creator && (
            <div className="flex items-center">
              ...
              Created by {playlist.creator.fullName || playlist.creator.username}
            </div>
          )}
          <div className="flex items-center">
      ...
      {playlist.movies.length === 0 ? (
        <div className="text-center py-12">
          <h3 className="text-xl font-medium mb-2">No movies in this playlist</h3>
                    ...
                    >
                      {(provided) => (
                        <Card 
                          ...
                          className={`overflow-hidden transition-all ${isReordering ? 'border-dashed border-2 cursor-grab' : ''}`}
                        >
                          ...
                          <CardContent className="p-4">
                            ...
                            {movie.playlistItem?.notes && (
                              <div className="mt-3 p-2 bg-muted rounded-md text-sm">
                                <p className="line-clamp-3">{movie.playlistItem.notes}</p>
                              </div>
                            )}
                          </CardContent>
                            ...
                            {isOwner && !isReordering && (
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  <DropdownMenuItem onClick={() => handleEditNotes(movie)}>
          ...
          <div className="space-y-4 py-4">
            {!playlist?.isPublic ? (
              <div className="text-center p-4 bg-muted rounded-md">
                ...
                <p className="text-sm">This playlist is private. Make it public to share it with others.</p>
                {isOwner && (
                  <div className="mt-4">
            ...
            {playlist?.isPublic && (
              <div className="flex flex-col space-y-2 mt-4">
                ...
                <Button className="w-full" variant="outline" asChild>
                  <a href={`https://twitter.com/intent/tweet?url=${encodeURIComponent(shareUrl)}&text=Check out this movie playlist: ${encodeURIComponent(playlist?.name)}`} target="_blank" rel="noreferrer">
